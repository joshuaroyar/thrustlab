...
>
	...
	>
		...
		<form
			method="POST"
			action="?/login"
			class="animate-on-scroll mb-6 flex flex-col gap-5 md:gap-6"
			use:enhance
		>
			{#if form?.error}
		...
		<button
			...
			class="font-body mb-6 flex w-full items-center justify-center gap-3 rounded-full bg-white py-3 text-base font-semibold text-[#3c4043] shadow-md transition-all hover:-translate-y-[2px] hover:bg-gray-50 hover:shadow-lg active:translate-y-0"
		>
		...
		<p class="font-body m-0 text-center text-[#E8F4FA]/90 transition-colors">
